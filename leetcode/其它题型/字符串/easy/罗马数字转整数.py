...
'''
思路1：将七个字符和特殊规则字符及其代表的整数存入在哈希表中
        遍历字符串，累加整数

大神思路：充分理解规则，让特殊规则也不特殊，哈希表只需存7个罗马字符
            若当前字符比下一个大，则加这个数，否则减这个数
'''
class Solution:
    #击败55%
    ...
    def romanToInt_1(self, s:str) -> int:
        ...
        for i in range(len(s)):
            #这里注意条件对应的语句不能写错，因为else否定的是两个条件，当i = len(s)-1时，这个数都是加
